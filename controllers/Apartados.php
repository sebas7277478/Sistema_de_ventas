...
class Apartados extends Controller
{
    ...
    public function registrarApartado()
    {
        ...
        if (!empty($datos['productos'])) {
            ...
            if (empty($idCliente)) {
                ...
                $res = array('msg' => 'LA FECHA APARTADO ES REQUERIDA', 'type' => 'warning');
            } else if (empty($fecha_retiro)) {
                $res = array('msg' => 'LA FECHA RETIRO ES REQUERIDA', 'type' => 'warning');
            } else if (empty($abono)) {
                $res = array('msg' => 'EL ABONO ES REQUERIDO', 'type' => 'warning');
            } else {
    ...
    public function procesarEntrega($idApartado)
    {
        ...
        if ($data == 1) {
            ...
            $res = array('msg' => 'PROCESADO CON EXITO', 'type' => 'success');
        } else {
    ...
    public function listarHistorial()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['acciones'] = '<a class="btn btn-danger" href="#"><i class="fas fa-file-pdf" onclick="verReporte(' . $data[$i]['id'] . ')"></i></a>';
        }
        echo json_encode($data);
        die();
    }
