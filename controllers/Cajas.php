...
class Cajas extends Controller
{
    ...
    public function registrarGasto()
    {
        if (isset($_POST['monto']) && isset($_POST['descripcion'])) {
            if (empty($_POST['monto'])) {
                ...
                $res = array('msg' => 'LA DESCRIPCIÓN ES REQUERIDA', 'type' => 'warning');
            } else {
    ...
    public function listarGastos()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            $data[$i]['foto'] = '
            <a href="' . BASE_URL . $data[$i]['foto'] . '" target="_blank">
            <img class="img-thumbnail" src="' . BASE_URL . $data[$i]['foto'] . '" width="100">
            </a>';
        }
        echo json_encode($data);
        die();
    ...
    {
        $consultaVentas = $this->model->getVentas('total', $this->id_usuario);
        $ventas = ($consultaVentas['total'] != null) ? $consultaVentas['total'] : 0;
    ...
    public function cerrarCaja()
    {
        ...
        if ($result == 1) {
            ...
            $res = array('msg' => 'ERROR AL CERRAR LA CAJA', 'type' => 'errors');
        }
        echo json_encode($res);
        die();
    }
