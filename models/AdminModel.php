...
class AdminModel extends Query{
    ...
    {
        $sql = "SELECT descripcion, cantidad FROM productos WHERE cantidad < 10 LIMIT 4";
        return $this->selectAll($sql);
    }    
    //reporte pdf
    public function minimosProductosPDF()
    {
        $sql = "SELECT p.*, c.categoria FROM productos p INNER JOIN categorias c ON p.id_categoria = c.id WHERE p.cantidad < 10";
        return $this->selectAll($sql);
    }
    ...
    {
        $sql = "SELECT SUM(cantidad*precio_compra) as resultado FROM productos";
        return $this->select($sql);
    }
}
?>