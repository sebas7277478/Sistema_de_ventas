...
class ApartadosModel extends Query
{
    ...
    public function registrarApartado($productos, $fecha_create, $fecha_apartado, $fecha_retiro, $abono, $total, $color, $idCliente, $id_usuario)
    {
        ...
        $array = array($productos, $fecha_apartado, $fecha_create, $fecha_retiro, $abono, $total, $color, $idCliente, $id_usuario);
    ...
    {
        $sql = "SELECT ap.*, cl.documento, cl.num_documento, cl.nombre, cl.telefono, cl.direccion FROM apartados ap INNER JOIN clientes cl ON ap.id_cliente = cl.id WHERE ap.id = $idApartado";
        return $this->select($sql);
    }
