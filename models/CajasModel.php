...
class CajasModel extends Query
{
    ...
    public function abrirCaja($monto, $fecha_apertura, $id_usuarios)
    {
        $sql = "INSERT INTO cajas (monto_inicial, fecha_apertura, id_usuarios) VALUES (?,?,?)";
        $array = array($monto, $fecha_apertura, $id_usuarios);
        return $this->insertar($sql, $array);
    }
    public function getCaja($id_usuarios)
    {
        $sql = "SELECT * FROM cajas WHERE estado = 1 AND id_usuarios = $id_usuarios";
        return $this->select($sql);
    }
    public function getCajas()
    {
        $sql = "SELECT c.*, u.nombre FROM cajas c INNER JOIN usuarios u ON c.id_usuarios = u.id";
    ...
    {
        $sql = "UPDATE cajas SET fecha_cierre=?, monto_final=?, total_ventas=?, egresos=?, gastos=?, estado=? WHERE estado = ? AND id_usuarios = ?";
